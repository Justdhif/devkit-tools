...
export function AiAssistantTool({ initialTab }: { initialTab?: AiTab }) {
  ...
  const tabs: { id: AiTab; label: string; icon: React.ReactNode }[] = [
    { id: 'regex', label: 'Regex Generator', icon: <RegexIcon className="w-3.5 h-3.5" /> },
    { id: 'sql', label: 'SQL Generator', icon: <Database className="w-3.5 h-3.5" /> },
    { id: 'error', label: 'Error Explainer', icon: <AlertTriangle className="w-3.5 h-3.5" /> },
    { id: 'code', label: 'Code Explainer', icon: <Code2 className="w-3.5 h-3.5" /> },
    { id: 'json', label: 'JSON Converter', icon: <FileCode className="w-3.5 h-3.5" /> },
  ];
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`relative flex items-center space-x-2 px-3 py-1.5 text-xs font-medium rounded-t-lg transition-colors ${
            ...
            >
              {isActive && (
                <motion.div
                  ...
                  className="absolute inset-0 bg-surface border-b-2 border-accent rounded-t-lg"
